...
my $get_posda_backlog = PosdaDB::Queries->GetQueryInstance("GetPosdaQueueSize");
...
my $usage = <<EOF;
Usage: FileImportIntoPosda.pl  <import_type> <comment>";
or: /FileImportIntoPosda.pl -h

Expects list of file_paths on STDIN
EOF
unless ($#ARGV == 1) {die $usage;}
if(-f "shutdown_import") {
  die "import_shutdown";
}
my $import_type = $ARGV[0];
my $comment = $ARGV[1];
...
print STDERR "$fname is not a file\n";
  ...
  $total_files += 1;
  my $posda_queue_size; 
  $get_posda_backlog->RunQuery(sub {
    my($row) = @_;
    $posda_queue_size = $row->[0];
  }, sub {});
  my $start_queue_size = $posda_queue_size;
  my $start_wait = time;
  throttle:
  while(1){
    if($posda_queue_size > 50){
      sleep 10;
      $get_posda_backlog->RunQuery(sub {
        my($row) = @_;
        $posda_queue_size = $row->[0];
      }, sub {});
      if($posda_queue_size <= 500){
        my $end_wait = time;
        my $total_elapsed = $end_wait - $script_start_time;
        my $wait_time = $end_wait - $start_wait;
        print STDERR "Waited $wait_time seconds for backlog to go from $start_queue_size to " .
          "$posda_queue_size\n";
        print STDERR "Files so far:  " .
          "$total_files (after $total_elapsed)\n";
        last throttle;
      }
    } else {
      last throttle;
...
my $end_run_time = time;
my $final_elapsed = $end_run_time - $script_start_time;
print STDERR "Finished after $final_elapsed seconds, $total_files files\n";
