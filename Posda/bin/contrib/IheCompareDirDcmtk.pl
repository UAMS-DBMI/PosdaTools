#!/usr/bin/perl -w
#$Source: /home/bbennett/pass/archive/Posda/bin/contrib/IheCompareDirDcmtk.pl,v $
#$Date: 2013/05/01 15:27:18 $
#$Revision: 1.1 $
#
...
for my $i (sort keys %FromInfo){
  if(exists $ToInfo{$i}){
    ...
    `dcmdump $FromInfo{$i}->{file} >$from_dump`;
    `dcmdump $ToInfo{$i}->{file} >$to_dump`;
