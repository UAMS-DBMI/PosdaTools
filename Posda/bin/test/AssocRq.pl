#!/usr/bin/perl -w
#$Source: /home/bbennett/pass/archive/Posda/bin/test/AssocRq.pl,v $
#$Date: 2009/11/18 14:15:07 $
#$Revision: 1.1 $
#
...
#
use strict;
use Posda::Ae::AssocRq;
use DBI;
my $config_dir = "$ENV{POSDA_HOME}/config/ae_test/assoc_rq";
...
for my $c (@configs){
  ...
  my $rname = $1;
  my $lae = Posda::Ae::AssocRq->from_config($c);
...
#  print FILE $pdu;
#  close FILE;
  my $lae_1 = Posda::Ae::AssocRq->from_pdu($pdu);
  ...
  $lae->DumpGuts(\*FILE);
  close FILE;
  my $lae_2 = Posda::Ae::AssocRq->from_db($db, $lae->{id});
