#!/usr/bin/perl -w
#$Source: /home/bbennett/pass/archive/Posda/bin/test/PixBitMapToContour.pl,v $
#$Date: 2012/01/25 16:08:33 $
#$Revision: 1.2 $
#
...
for my $i (0 .. $rows - 1){
  ...
  if($i < $rows){
    my $buff = "";
    while(length($buff) < $bytes_in_row){
      my $count = sysread(INPUT, $buff, $bytes_in_row - length($buff), length($buff));
      unless($count) {
        die "$0: premature end of read ($count vs $bytes_in_row)\n" .
         " row: $i\n" .
