...
my $usage = <<EOF;
StreamingPosdaPublicCompareSops.pl <compare_public_to_posda_instance_id>
or
StreamingPosdaPublicCompareSops.pl -h

Populates posda_public_compare table
Expect input lines in following format:

<sop_instance_uid>|<posda_file_id>|<posda_file_path>|<public_file>
EOF
...
#public_to_posda_file_comparison
...
#print STDERR "Received Compare Instructions: $line\n";
  my($sop_inst, $from_file_id, $from_file, $to_file) =
    split(/\|/, $line);
...
#  my($s_rept, $l_rept) = $diff->DiffReport;
  ...
  $ins->RunQuery(sub{}, sub{},
    $cmp_id, $sop_inst, $from_file_id, $from_file, $to_file, $s_rept_file_id, $l_rept_file_id);
  unless(defined $s_rept_file_id) { $s_rept_file_id = "<undef>" }
  unless(defined $l_rept_file_id) { $s_rept_file_id = "<undef>" }
  print "Completed: $sop_inst|$from_file_id|$from_file|" .
