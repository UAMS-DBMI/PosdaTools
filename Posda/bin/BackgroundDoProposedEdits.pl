...
my $usage = <<EOF;
Usage:
BackgroundDoProposedEdits.pl <bkgrnd_id> <dest_root> <who> <edit_desciption> <notify>
or
BackgroundDoProposedEdits.pl -h
...
Note: if you use hash_unhashed_uid, and you have uid_substitutions, the
...
while(my $line = <STDIN>){
  chomp $line;
  my($unmap, $map, $series_uid, $num_files, $op, $tag, $v1, $v2) = 
    split(/&/, $line);
  if($unmap && $map){
...
#############################
# Now die unless the destination directory exists
my $num_sops = keys %SopsToEdit;
print "Found list of $num_sops to edit\nChecking destination directory\n";
unless(-d $DestDir){
  print "Error: $DestDir does not exist or is not a directory\n";
  die "Error: $DestDir does not exist or is not a directory";
}
#############################
# This is code which sets up the Background Process and Starts it
my $background = Posda::BackgroundProcess->new($invoc_id, $notify);
print "Directory $DestDir exists\nForking background process\n";
$background->Daemonize;
# now in the bakground...
...

{
  ...
  sub new{
    ...
    if(defined($uid_map)){ $this->{uid_map} = $uid_map }
    bless($this, $class);
    my $at_text = $this->now;
