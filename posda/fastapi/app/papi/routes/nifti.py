...
@router.put("/set_edit/{nifti_id}/{good_status}/{user}")
async def set_edit(nifti_id: int, good_status: bool ,user: str, db: Database = Depends()):
    record = await db.fetch("""\
        INSERT INTO nifti_visual_review_status
        VALUES($1, $2, $3, now());
        """, [nifti_id, good_status, user])
