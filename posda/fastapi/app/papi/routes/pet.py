...
@router.get("/find_pets_in_activity/{activity_id}")
async def find_pets_in_activity(activity_id: int, db: Database = Depends()):
    query = """\
        select
          file_id,
        ...
        where file_id in (
          ...
          where
          dicom_file_type = 'Positron Emission Tomography Image Storage'
