  ...
  for my $study(keys %{$Hierarchy{$pat}}){
    for my $series(keys %{$Hierarchy{$pat}->{$study}}){
      ...
      $files_in_series->RunQuery(sub {
        ...
        unless($result) { $files_copied += 1; $total_files_copied += 1 }
      }, sub {}, $series);
      $background->WriteToEmail("$files_copied for series $series\n");
    }
  }
}
...
$background->WriteToEmail("$total_files_copied files copied under directory:" .
   "$to_dir\n");
$background->Finish;
