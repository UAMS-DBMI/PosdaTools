...
for my $line (@lines){
  ...
  $p->RunQuery(sub{
    ...
    %RepeatedPatientMapping{$line}->{$from_patient_id} = {
      from_patient_id_e => $from_patient_id,
      to_patient_id_e => $to_patient_id,
      to_patient_name_e => $to_patient_name,
      collection_name_e => $collection_name,
      site_name_e => $site_name,
      batch_number_e => $batch_number,
      diagnosis_date_e => $diagnosis_date,
      baseline_date_e => $baseline_dat,
      date_shift_e => $date_shift,
      uid_root_e => $uid_root,
      site_code_e => $site_code
    };, sub {}, $from);
