...
def saveTiffMetaData(mytif, activity_id, file_id):
    phi_scan_id = Query("CreateTiffPHIScan").get_single_value(description = "Tiff Phi scan for Activity {0}".format(activity_id))
    for p, page in enumerate(mytif.pages):
        for t, tag in enumerate(page.tags):
            ...
            # tag over 32768 are private
            if not tag_seen_id:
                tag_seen_id = Query("InsertTiffTagSeen").get_single_value(is_private=False,tag_name = tag.name)
            value_seen_id = Query("GetTiffValueSeen").get_single_value(value = tag.navalueme)
...
def main(args):
    ...
    for (file_id, svsfilepath) in results:
        ...
        if (myfilename[-3:].lower() == "svs" or myfilename[-3:].lower() == "tif" or myfilename[-4:].lower() == "tiff"):
            ...
            saveTiffMetaData(mytif, args.activity_id, file_id)
