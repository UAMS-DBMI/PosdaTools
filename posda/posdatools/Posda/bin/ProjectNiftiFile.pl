...
my $dir = getcwd;
my $usage = <<EOF;
ProjectNiftiFile.pl <file_id> <file_name> <to_dir>
EOF
...
unless($#ARGV == 2){
  my $num_args = @ARGV;
  die "Wrong number args ($num_args vs 3)\n$usage";
}
my($file_id, $file, $to_dir) = @ARGV;
unless ($file =~ /^\//){
  $file = "$dir/$file";
}
my $nifti = Nifti::Parser->new($file);
my($rows, $cols, $depth) = $nifti->RowsColsAndBytes;
my $gray_avg = "$to_dir/nifti_$file_id" . "_p_avg.gray";
my $gray_min = "$to_dir/nifti_$file_id" . "_p_min.gray";
my $gray_max = "$to_dir/nifti_$file_id" . "_p_max.gray";
my $jpeg_avg = "$to_dir/nifti_$file_id" . "_p_avg.jpeg";
my $jpeg_min = "$to_dir/nifti_$file_id" . "_p_min.jpeg";
my $jpeg_max = "$to_dir/nifti_$file_id" . "_p_max.jpeg";
