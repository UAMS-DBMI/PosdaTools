...
my $usage = <<EOF;
Usage:
RadcompXlsConverter.pl <bkgrnd_id> <notify>
or
RadcompXlsConverter.pl -h
...
for my $file_id (keys %Files){
  ...
  unless($Files{$file_id}->{file_type} eq "xls"){
    print "$file_id has wrong type (" .
...
$create_conversion->RunQuery(sub{}, sub{}, $notify,
  "XlsConverter.pl $invoc_id $notify");
my $conversion_id;
...
for my $file_id (keys %Files){
  ...
  my $cmd = "xls2csv -x \"$from_file\" -c \"$to_temp\"";
  `$cmd`;
  ...
  $create_ndf_change->RunQuery(sub {}, sub {},
    ...
    "XlsConverter($invoc_id, $notify)"
  );
  $update_ndf->RunQuery(sub {}, sub {},
    $from_file_type, $file_sub_type, $collection, $site,
    $subject, "converted_to_new_format (csv)",
    $file_id
  );
  $create_ndf->RunQuery(sub {}, sub {},
    $to_file_id, "csv", $file_sub_type, $collection,
    $site, $subject);
