...
my $q_inst = PosdaDB::Queries->GetQueryInstance("PrivateTagValuesWithVrTagAndCountWhereDispositionIsNull");
...
sub MakeProcessRow{
  ...
  my $process_row = sub {
    ...
    $Info->{$sag}->{$vr}->{private_disp} = $pdisp;
...
print '"Tag","Dispostion","VR","Name Chain","VR Chain","Disposition Chain"' . "\r\n";
for my $tag (sort keys %Info){
  my $t_print = $tag;
  $t_print =~ s/"/""/g;
  for my $vr (sort keys %{$Info{$tag}}){
    print "\"-$t_print-\"," .
      "\"$Info{$tag}->{$vr}->{private_disp}\"," .
      "\"$vr\"," .
      "\"$Info{$tag}->{$vr}->{name}\"," .
      "\"$Info{$tag}->{$vr}->{vr}\"," .
