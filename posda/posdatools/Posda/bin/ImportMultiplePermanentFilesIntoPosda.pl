...
my $usage = <<EOF;
Usage: ImportMultiplePermanentFilesIntoPosda.pl <comment>";
or: ImportMultiplePermanentFilesIntoPosda.pl -h

expects a list of files on STDIN;
  These will be imported into posda and then NOT DELETED or disturbed.
...
Query('InsertEditImportEvent')->RunQuery(
  sub{}, sub{}, "ImportMultiplePermanentFilesIntoPosda.pl", $comment);
####GetImportEventId
my $ie_id;
