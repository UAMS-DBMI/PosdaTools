...
my $usage = <<EOF;
# CmdCompressedPixBitMapToContour.pl <rows> <cols>
#
# This program accepts a compressed bitmap on STDIN
# and writes contours to STDOUT

#   rows=<number of rows in bitmap input>
#   cols=<number of cols in bitmap input>
#
#  Contours will be written to STDOUT in following format:
...
my $num_args = @ARGV;
unless($#ARGV == 1) { die "Wrong # args ($num_args vs 1)\n$usage" }
my($rows, $cols) = @ARGV;
my $debug = 0;
...
for my $i (0 .. $rows + 1){
  ...
  if($i < $rows){
    ...
    while($#row_in_constr < $bytes_in_row - 1){
      my $byte_buff;
      if($cur_count <= 0){
        my $count = sysread(STDIN, $byte_buff, 1);
        if($count != 1){
...
if ($debug) {
  print STDERR "$0: Finished writing contours.\n";
}
