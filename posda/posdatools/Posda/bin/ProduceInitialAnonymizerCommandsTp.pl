...
This script does not expect lines on STDIN
...
my $activity_timepoint_id = $res->{activity_timepoint_id};

# Get patient list from TP
...
#
for my $row (@$patients) {
  my ($patient_id, $series_instance_uid) = @$row;
...
$get_patient_mapping->RunQuery(sub{
  ...
  my($from_patient_id, $to_patient_id, $to_patient_name,
    $collection_name, $site_name, $batch_number, $date_shift, $diagnosis_date,
    $baseline_date, $year_of_diagnosis, $computed_date_shift) = @$row;
  ...
  if(defined $hash->{date_shift}){
    if($hash->{date_shift} =~ /^(.*) days$/){
       $hash->{date_shift} = $1;
    } elsif ($hash->{date_shift} == "00:00:00"){
      $hash->{date_shift} = 0;
    } else {
...
$rpt->print(
  "series_instance_uid,op,tag,val1,val2,Operation,activity_id,edit_description,notify\n");
my $sent_commands = 0;
...
#{{{ Delete Groups and make sure CTP Group is mapped
  ...
  $rpt->print(",delete_matching_group,\"60xx\",<>,<>\n");
  $rpt->print(",delete_matching_group,\"50xx\",<>,<>\n");
