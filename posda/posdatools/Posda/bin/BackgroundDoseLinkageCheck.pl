...
my $usage = <<EOF;
DoseLinkageCheck.pl <bkgrnd_id> <notify_email>
or
...
print STDERR "$num_files files to process in child\n";
{
  ...
  $background->WriteToEmail("At: $date\nEntering background:\n" .
    "Script: Starting BackgroundDoseLinkageCheck.pl\n");
...
for my $i (0 .. $#Files){
  ...
  if(@Rows <= 0){
    ...
    $background->WriteToReport("\"$file_info->{collection}\"," .
      ...
      "\"$file_info->{series}\"," .
      "\"$file_id\"," .
      "\"File references more than one plan (????)\"," .
      "\r\n");
    next File;
  }
  my $ref = $Rows[0];
  ...
  if(@RefInfo <= 0){
    $background->WriteToReport("\"$file_info->{collection}\"," .
      ...
      "\"$file_info->{series}\"," .
      "\"$file_id\"," .
      "\"File references a plan which is not known\"" .
      "\r\n");
    next File;
  } elsif(@RefInfo > 1){
    $background->WriteToReport("\"$file_info->{collection}\"," .
      ...
      "\"$file_info->{series}\"," .
      "\"$file_id\"," .
      "\"File references a plan with duplicate SOP instance UIDs\"" .
      "\r\n");
    next File;
  }
  my $ref_plan_info = $RefInfo[0];
  my $ref_file_id = $ref_plan_info->[0];
  my $ref_collection = $ref_plan_info->[1];
  my $ref_site = $ref_plan_info->[2];
  my $ref_patient_id = $ref_plan_info->[3];
  my $ref_sop_class_desc = $ref_plan_info->[4];
  my $ref_modality = $ref_plan_info->[5];
  my $ref_sop_class_uid = $ref_plan_info->[6];
  my $ref_series_uid = $ref_plan_info->[7];
  my @errors;
