...
for my $f (keys %FileIds){
  ...
  my $col_names = [
   ...
   "series_instance_uid", "sop_instance_uid", "pixel_offset",
  ];
  Query("GetSegBitmapByFileId")->RunQuery(sub {
    my($row) = @_;
  ...
  $parms->{num_slices} = keys %{$seg_obj->{frame_descriptor}};
  ...
  if(defined $seg_bitmap_file_id){ # Check seg_bitmap_file table against parameters
    ...
    $seg_bitmap_file_id = $f;
    Query("InsertSegBitmapFileId")->RunQuery(sub {}, sub {}, 
  ...
  Query('GetSegBitmapSegmentations')->RunQuery(sub  {
    my($row) = @_;
    for my $i (0 .. $#col_names){
      $DbStruct{$row->[1]}->{$col_names->[$i]} = $row->[$i];
    }
  }, sub {}, $f);
  my %FileStruct;
  ...
  if($segs_in_db > 0){## Check DB against file
    ...
    for my $seg_no (keys %$a){
      unless(ref($a->{seg_no}) eq "HASH"){
        push @seg_err, "$a_cap has seg_no with bad content";
        next aseg;
      }
    ...
    for my $seg_no (keys %$b){
      unless(ref($b->{seg_no}) eq "HASH"){
        push @seg_err, "$b_cap has seg_no with bad content";
        next bseg;
      }
    ...
    for my $seg_no (keys %FileStruct){
      ...
      push @args, $f;
      push @args, $seg_no;
      for my $prop (@col_names){ push @args, $FileStruct{$prop} };
  ...
  for my $i (keys %{$seg_obj->{frame_descriptor}}){
    for my $j (keys %{$seg_obj->{frame_descriptor}->{$i}}){
      my $frame = $seg_obj->{frame_descriptor}->{$i}->{$j};
      ...
      ## Check/Populate seg_slice_bitmap_file
      my @cols = [ "seg_slice_bitmap_file_id", "seg_bitmap_file_id",
        "segmentation_number", "iop", "ipp",
        "total_ones", "num_bare_points"];
      my $num_cols = @cols;
      my %f_parms;
      $f_parms{segmenation_number} = $frame->{referenced_segment_number};
      ...
      my %db_struct;
      Query('GetSegSliceBitmapFileBySegBitmapFileIdAndSliceFileId')->RunQuery(sub{
        my($row) = @_;
        for my $i (0 .. $num_cols){
      ...
      my %db_sops;
      Query('GetSliceBitmapFileRelatedSop')->RunQuery(sub{
        my($row) = @_;
        $db_sops{$row->[0]} = 1;
      }, sub{}, $slice_file_id, $f);
      ...
      if($num_db_sops > 0){## Compare db to file
        ...
        for my $sop(keys %{$frame->{referenced_image}}){
         $q->RunQuery(sub{}, sub{}, $slice_file_id, $f, $sop);
        }
      }
      ...
      if($num_db == 0 && $num_f > 0){## insert db
        ...
        for my $p (keys %f_pts){
          $q->RunQuery(sub{}, sub{}, $slice_file_id, $f, $p);
        }
