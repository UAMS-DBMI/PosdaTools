...
my $usage = <<EOF;
CopyPriorTimepointExcludingFiles.pl <?bkgrd_id?> <activity_id> <old_tp_id> <notify>
or 
CopyPriorTimepointExcludingFiles.pl -h

Expects a list of file_id's to be excluded on STDIN:
<file_id>
...

Creates a new timepoint with all of the files in the old timepoint except those excluded
...
while(my $line = <STDIN>){
  chomp $line;
  $ExcludedFileIds{$line} = 1;
}
my $num_exclusions = keys %ExcludedFileIds;

print "$num_exclusions files to be excluded\n";
print "Going to background to copy files with exclusions\n";
...
for my $file_id (keys %AllFilesInOld){
  if(exists $ExcludedFileIds{$file_id}){
    $num_excluded += 1;
  } else {
    $num_copied += 1;
