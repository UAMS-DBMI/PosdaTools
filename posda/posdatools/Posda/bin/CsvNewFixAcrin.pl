...
use Time::Piece;
my $usage = <<EOF;
CsvNewFixAcrin.pl <invoc_id> <to_dir> <uid_root> <low_date> <high_date> <notify>
...
Expects the following list on <STDIN>
  <patient_id>&<study_uid>&<series_uid>&<file_id>&<offset_pat_id>&<offset>
...
Actually invokes NewFixAcrin.pl to do the edits
...
while(my $line = <STDIN>){
  chomp $line;
#print "line: $line\n";
  my($patient_id, $study_uid, $series_uid, $sop_instance_uid, $file_id, $offset_pat_id, $offset) =
    split /&/, $line;
  if(defined($series_uid) && $series_uid ne ""){
    $Patients{$patient_id}->{$study_uid}->{$series_uid}->{$sop_instance_uid}->{$file_id} = 1;
    $Series{$series_uid} = 1;
  }
...
my $get_file_path = PosdaDB::Queries->GetQueryInstance(
  "GetFilePathAndModality");
patient:
for my $patient_id (sort keys %Patients){
  for my $study_uid (sort keys %{$Patients{$patient_id}}){
    for my $series_uid (sort keys %{$Patients{$patient_id}->{$study_uid}}){
      ...
      my $file_id;
      for my $sop_instance_uid (
        keys %{$Patients{$patient_id}->{$study_uid}->{$series_uid}}
      ){
        my @file_ids = keys
          %{$Patients{$patient_id}->{$study_uid}->{$series_uid}->{$sop_instance_uid}};
        if($#file_ids != 0){
          $background->WriteToEmail("Error: $sop_instance_uid has multiple " .
            "file_ids\n");
          next patient;
        }
        my $file_id = $file_ids[0];
        my $path;
        my $modality;
        ...
        my $cmd = "NewFixAcrin.pl $path \"$to_dir/$patient_id/" .
          "$study_uid/" .
...
while(1){
  ...
  #$background->WriteToEmail("1. Running cmd: $cmd\n");
  print SCRIPT1 "$cmd\n";
  $cmd = shift @cmds;
  unless(defined $cmd){ last command }
  print STDERR "2. Running cmd: $cmd\n";
  #$background->WriteToEmail("2. Running cmd: $cmd\n");
  print SCRIPT2 "$cmd\n";
  $cmd = shift @cmds;
  unless(defined $cmd){ last command }
  print STDERR "3. Running cmd: $cmd\n";
  #$background->WriteToEmail("3. Running cmd: $cmd\n");
  print SCRIPT3 "$cmd\n";
  $cmd = shift @cmds;
  unless(defined $cmd){ last command }
  print STDERR "4. Running cmd: $cmd\n";
  #$background->WriteToEmail("4. Running cmd: $cmd\n");
  print SCRIPT4 "$cmd\n";
  $cmd = shift @cmds;
  unless(defined $cmd){ last command }
  print STDERR "5. Running cmd: $cmd\n";
  #$background->WriteToEmail("5. Running cmd: $cmd\n");
  print SCRIPT5 "$cmd\n";
}
$background->WriteToEmail(`date`);
