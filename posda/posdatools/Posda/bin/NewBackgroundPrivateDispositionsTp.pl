...
First verifies that every patient in the time point has a row in the patient_mapping table.
If not, it constructs are report of all of the patient mapping rows with values for those
that exist, and with blanks for those that need to be created.

If patient mapping exists for all patients, constructs a destination file name as follows:
...
This script also populates public_copy_status to feed the files to the "Steve" based
process which imports the files into Public.
EOF
...
if (not defined $site_code) {
  ...
  $background->Finish("Error no entry for $site_name in site_codes table!");
  exit;
}
...
if($num_errors > 0){
  ...
  $background->Finish("There were $num_errors in tp series");
  exit;
}
...
for my $pat (keys %Patients){
  $q3->RunQuery(sub{
    ...
    if(defined $computed_shift){
      if($computed_shift eq "00:00:00") { $computed_shift = "0 days" }
      if($computed_shift =~ /^([^\s]+)\s*days$/){
        $shift = $1;
      }
    } elsif(defined $date_shift){
      if($date_shift eq "00:00:00") { $date_shift = "0 days" }
      if($date_shift =~ /^([^\s]+)\s*days$/){
        $shift = $1;
      }
    }
    unless(defined $shift) {
      $background->WriteToEmail("$pat has no dateshift\n");
      $error += 1;
    }
    my $offset = $shift;
...
if(@new_tags > 0){
  ...
  $background->Finish("Error: private tags with no disposition");;
  exit;
}
...
if($error){
  ...
  $background->Finish("Error: terminating because $error errors found");;
  exit;
}
...
#open SCRIPT9, "|/bin/sh";
#open SCRIPT0, "|/bin/sh";
my $total_to_submit = @cmds;
command:
while(1){
  my $files_remaining = @cmds;
  $background->SetActivityStatus("Running $files_remaining remaining of $total_to_submit");
  my $cmd = shift @cmds;
...
$background->SetActivityStatus("All $total_to_submit files submitted - waiting for subshells to clear");
close SCRIPT1;
close SCRIPT2;
close SCRIPT3;
close SCRIPT4;
...
$background->Finish("($total_to_submit Dispositions Applied in $duration seconds");
