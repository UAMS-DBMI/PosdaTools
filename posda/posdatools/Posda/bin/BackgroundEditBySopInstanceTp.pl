...
my $usage = <<EOF;
Usage:
BackgroundEditBySopInstanceTp.pl <?bkgrnd_id?> <activity_id> <desciption> <notify>
or
BackgroundEditBySopInstanceTp.pl -h

Expects lines of the form:
<unmapped_uid>\x1C<mapped_uid>\x1C<sop_instance_uid>\x1C<operation>\x1C<tag>\x1C<val1>\x1C<val2>
...
my $last_line_was_sop = 0;
my $last_line_was_edit = 0;
my $accumulating_sops = [];
my $current_sops = [];
...
while(my $line = <STDIN>){
  chomp $line;
  $line_no += 1;
  my($unmap, $map, $sop_uid, $op, $tag, $v1, $v2) = split("", $line);
  if($unmap && $map){
    $UidMapping{$unmap} = $map;
  }
  if($sop_uid){
    if($op) {
      print "Error: operation ($op) on same line as sop ($sop_uid)\n";
      exit;
    }
    if($last_line_was_edit){
#print "Calling ProcessEndOfEdits(1)\n";
      ProcessEndOfEdits();
#print "Clearing $current_sops\n";
      $current_sops = [];
    }
    push @$accumulating_sops, $sop_uid;
#print "accumulated sop: $sop_uid\n";
    $sops_seen{$sop_uid} = $line_no;
    $last_line_was_sop = 1;
    $last_line_was_edit = 0;
    next line;
  }
  if($op){
    if($last_line_was_sop) {
#print "Set current_sops\n";
      $current_sops = $accumulating_sops;
#print "Cleared accumulating_sops\n";
      $accumulating_sops = [];
#my $num_ac_sops = @$accumulating_sops;
#print "$num_ac_sops remaining\n";
      unless($#{$current_sops} >= 0){
        print "Error: operation($op) applies to no sop\n";
        exit;
      }
    }
...
#print "Here we are!!\n";    
#print "Calling ProcessEndOfEdits(2)\n";
    ProcessEndOfEdits();
    $last_line_was_edit = 0;
  }
}
if($last_line_was_edit) {
print "Calling ProcessEndOfEdits(3)\n";
 ProcessEndOfEdits();
}

#############################
# Process a list of Edits with sop list
sub ProcessEndOfEdits{
print "ProcessEndOfEdits\nSops:\n";

#for my $sop(@$current_sops){
#  print "\t$sop\n";
#}
  my @sops_for_these_edits;
  for my $sop(@$current_sops){
    my($from_file_id, $from_file_path, $to_file_path) = 
      GetFromToFileForSopActivity($sop, $activity_id);
    if(exists $SopsToEdit{$sop}){
      print "Error: the sop ($sop) " .
        "seems to have occurred earlier at line $sops_seen{$sop}\n";
      exit;
    }
    push @sops_for_these_edits, $sop;
print "$from_file_path => $to_file_path\n";
    $SopsToEdit{$sop}->{from_file} = $from_file_path;
    $SopsToEdit{$sop}->{to_file} = $to_file_path;
  }
...
print "Cleared accumulating_edits\n";
  $accumulating_edits = [];
}
sub GetFromToFileForSopActivity{
  my($sop_instance_uid, $activity_id) = @_;
  my($file_id, $path);
  Query('GetFromToFileForSopActivity')->RunQuery(sub {
    my($row) = @_;
    if(defined($file_id) && $file_id != $row->[0]){
      print "Error: SOP ($sop_instance_uid) returned " .
        "two files $file_id and $row->[0] for " .
        "activity $activity_id\n";
      exit;
    }
    $file_id = $row->[0];
    $path = $row->[1];
  }, sub {}, $sop_instance_uid, $activity_id);
  unless(defined $file_id) {
    print "No file_id returned for $sop_instance_uid, $activity_id\n";
    exit;
  }
  my $sub_dir;
  if($file_id =~ /(\d\d)$/){
  ...
  unless(defined $sub_dir) { print "???\n"; die "???" }
  unless(-d "$DestDir/$sub_dir"){
    unless((mkdir "$DestDir/$sub_dir") == 1){
      print "Couldn't mkdir $DestDir/$sub_dir ($!)\n";
      exit;
    }
    print "Created directory $DestDir/$sub_dir\n";
  }
  my $to_file = "$DestDir/$sub_dir/$file_id.dcm";
  return ($file_id, $path, $to_file);
}
#############################
