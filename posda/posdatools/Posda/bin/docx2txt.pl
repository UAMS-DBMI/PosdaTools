...
# docx2txt - convert docx documents to text format
# Sandeep Kumar
# tweaked by Steve Kinzler, steve@kinzler.com, Oct 11
# http://docx2txt.sourceforge.net/
# http://kinzler.com/me/home.html#other
...
our $unzip = 'unzip';		# Windows path like 'C:/path/to/unzip.exe'
our $newLine = "\n";		# Alternative is "\r\n".
our $listIndent = "  ";		# Indent nested lists by "\t", " " etc.
our $lineWidth = 75;		# Line width, used for short line justification.
our $showHyperLink = "Y";	# Show hyperlink alongside linked text.
...
# kinzler 2011-10-24 kludge
if ($ARGV[0] eq '-') {
	umask 077;
	$tmp = "/tmp/docx2txt.$$";
	$SIG{'HUP'} = $SIG{'INT'} = $SIG{'PIPE'} = $SIG{'TERM'} = 'end';
	`cat > $tmp`;
	$ARGV[0] = $tmp;
	sub end { unlink $tmp; exit }
}
...
die $usage if (@ARGV == 0 || @ARGV > 2) || $ARGV[0] eq '-h';
...
$content =~ s|<w:checkBox><w:sizeAuto/><w:default w:val="([0-9]+)"/>|$listIndent x $1 . "($levchar[$1]) "|oge;
