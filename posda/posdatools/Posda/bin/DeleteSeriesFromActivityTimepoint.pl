...
my $usage = <<EOF;
Usage:
DeleteSeriesFromActivityTimepoint.pl <?bkgrnd_id?> <activity_id> <comment> <notify>
  or
DeleteSeriesFromActivityTimepoint.pl -h
...
my %SeriesInOldTp;
###### end Globals
while(my $series = <STDIN>){
  chomp $series;
  $SeriesToDelete{$series} = 1;
}
my $start = time;
...
for my $series_instance_uid (keys %SeriesInOldTp){
  if(exists $SeriesToDelete{$series_instance_uid}) { next series }
...
for my $i (keys %FilesInOldTp){
  unless(exists $NewFiles{$i}){
     $FilesDeleted{$i} = 1;
  }
}
...
my $files_deleted = keys %FilesDeleted;
...
print "Deleted Files $files_deleted\n";
#############################
