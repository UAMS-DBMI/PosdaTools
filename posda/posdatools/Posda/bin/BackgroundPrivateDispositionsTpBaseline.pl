...
my $usage = <<EOF;
BackgroundPrivateDispositionsTpBaseline.pl <?bkgrnd_id?> <activity_id> <notify> <skip_dispositions>
...
my($invoc_id, $act_id, $notify, $skip_dispositions) = @ARGV;
...
$background->WriteToEmail("<a onclick=\"javascript:event.target.port=80\" href=\"/papi/v1/send_to_public_status/report/$invoc_id?pretty=1\">Public Copy Status Report</a>\n");
$background->Finish;
