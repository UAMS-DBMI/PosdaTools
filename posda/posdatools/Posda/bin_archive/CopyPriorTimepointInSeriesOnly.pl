...
my $usage = <<EOF;
 CopyPriorTimepointInSeriesOnly.pl <?bkgrnd_id?> <activity_id> <old_tp_id> <notify>
or 
 CopyPriorTimepointInSeriesOnly.pl -h

Expects a list of series_instance_id's to be excluded on STDIN:
<series_instance_uid>
...

Creates a new timepoint with all of the files in the old timepoint except those not
in one of the specified series
...
while(my $line = <STDIN>){
  chomp $line;
  $IncludedSeries{$line} = 1;
}
my $num_series = keys %IncludedSeries;

print "$num_series series to be include\n";
print "Going to background to copy files\n";
...
for my $file_id (keys %AllFilesInOld){
  ...
  unless(defined $series){
    $bg->WriteToEmail("Unable to get series for file $file_id\n");
    next file;
  }
  if(exists $IncludedSeries{$series}){
    $num_copied += 1;
  ...
  $bg->SetActivityStatus("Of $num_files in old tp, " .
    "$num_copied copied to new ($num_excluded excluded)");
}
