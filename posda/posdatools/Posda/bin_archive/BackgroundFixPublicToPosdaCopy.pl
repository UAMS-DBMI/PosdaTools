...
my $usage = <<EOF;
BackgroundFixPublicToPosdaCopy.pl <bkgrnd_id> <copy_id> <why> <notify>
or
BackgroundFixPublicToPosdaCopy.pl -h
...
my $start_time = `date`;
chomp $start_time;
$background->WriteToEmail("Starting BackgroundFixPublicToPosdaCopy.pl at " .
  "$start_time\n");
...
my $tt = `date`;
chomp $tt;
unless($rows_to_hide == 0 && $rows_to_copy > 0){
  $background->WriteToEmail("$tt: Not entering loop\n" .
    ...
    "This script is only for when there are files to copy\n" .
    "but none to hide\n"
  );
...
my $get_some_copies = Query("GetNfilesToCopyOnly");
...
while(1){
  ...
  $get_some_copies->RunQuery(sub {
    my($row) = @_;
    my $sop = $row->[0];
    my $copy_file_path = $row->[1];
    unless(-f $copy_file_path){
      $background->WriteToEmail("Public file not found: $copy_file_path\n");
      $set_status->RunQuery(sub{}, sub {},
  ...
  for my $sop (keys %BySop){
    my $copy_file_path = $BySop{$sop}->{copy_file_path};
    my $collection = $BySop{$sop}->{project};
    my $site = $BySop{$sop}->{site};
    my $site_id = $BySop{$sop}->{site_id};
    print MOVE "$collection&$site&$sop&$copy_file_path\n";
  }
  close MOVE;
#  $set_status->RunQuery(sub{}, sub {},
#     "waiting for start", undef, $copy_id);
