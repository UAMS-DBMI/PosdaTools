...
select
  ...
  creation_time, e.start_time, e.end_time request_pending, request_status, a.status_text as activity_task_status,
  count(distinct file_id) as num_files
from
  export_event e join file_export using(export_event_id)
  join activity_task_status a using(subprocess_invocation_id)
where
  e.start_time is not null and e.end_time is not null and e.dismissed_time is null
group by
  export_event_id, activity_id, subprocess_invocation_id, export_destination_name,
  creation_time, e.start_time, e.end_time, request_pending, request_status, activity_task_status