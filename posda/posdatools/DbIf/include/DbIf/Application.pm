...
sub TagTest {
  my ($self, $query_tags_listref, $all_tags) = @_;
  my $query_tags = {};
...
sub DrawQueryModificationButtons {
  my ($self, $http, $dyn) = @_;
  if ($self->{user_has_permission}('superuser')) {
    $self->NotSoSimpleButton($http, {
      op => 'DeleteQuery',
...
sub SmallInputBoxWithAddButton {
  my ($self, $http, $dyn) = @_;
  my $id = $dyn->{id};
  my $op = $dyn->{op};
  my $extra = $dyn->{extra};
...
sub QueryEnd {
  my ($self, $query, $then, $invoked_id) = @_;
  my $start_time = time;
  my $sub = sub {
...
sub NicknamesByStudy {
  my ($self, $study_where, $nn_type, $table_n) = @_;
...
sub StudyNnsFetchedByStudy {
  my ($self, $nn_type, $table_n) = @_;
  my $sub = sub {
    my($status, $struct) = @_;
...
sub NicknamesByFileId {
  my ($self, $f_where, $nn_type, $table_n) = @_;
  my $q = {
    query => "select\n" .
...
sub FileNnsFetched {
  my ($self, $nn_type, $f_info, $table_n) = @_;
  my $sub = sub {
    my($status, $struct) = @_;
    if($status eq "Succeeded" && $struct->{Status} eq "OK"){
      my %dig_info;
...
sub SeriesStudyBySop {
  my ($self, $sop_where, $nn_type, $table_n) = @_;
  my $q = {
    schema => "posda_files",
    db_type => "postgres",
    query => "select\n" .
...
sub NicknamesBySop {
  my ($self, $sop_where, $sop_info, $file_info, $dig_info, $nn_type, $table_n) = @_;
  my $q = {
    query => "select\n" .
