...
-- Columns: ['element', 'vr', 'q_value', 'tag_name', 'num_series']
-- Args: ['scan_id']
...
select 
  distinct '<' || element_sig_pattern || '>' as element, vr, '<' || value || '>' as q_value, tag_name, count(*) as num_series
...
group by element, vr, q_value, tag_name
