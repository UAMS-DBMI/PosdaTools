#!/usr/bin/perl -w
#$Source: /home/bbennett/pass/archive/TciaCuration/bin/ExtractAndAnalyzeFromDb.pl,v $
...
use vars qw( $FileManager $DB $Collection $Site $RootDir $Dir $C_Dir $A_Dir 
  $InfoFile $BuildInfoFile $Start );
my $usage =
    "Usage:\n" .
    "ExtractAndAnalyzeFromDb.pl <db> <collection> <site> <R_dir> <C_dir> " .
    "<A_dir>\n";
unless($#ARGV == 5){
  print STDERR $usage;
  print "Error: must have 6 args\n";
  exit;
}
$Start = time;
$DB = $ARGV[0];
...
}
{
  ...
  sub StartProcessing{
    ...
    $this->{QueryReader} = Dispatch::LineReader->new_cmd(
      "ExtractAllCollection.pl \"$main::Collection\" \"$main::Site\" " .
      "\"$main::DB\"",
      $this->QueryLine,
      $this->EndQuery
    );
