...
class DownloadFile(HTTPMethodView):
    async def get(self, request, downloadable_file_id, hash):
        query = """
            select
...
class DownloadDir(HTTPMethodView):
    async def get(self, request, downloadable_dir_id, hash):
        query = """
            select
                path
...
blueprint.add_route(DownloadFile.as_view(), 
                    '/file/<downloadable_file_id>/<hash>') 
blueprint.add_route(DownloadDir.as_view(),
